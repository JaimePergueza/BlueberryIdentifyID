...
- El workflow de CI (`.github/workflows/tests.yml`) tiene desde la Fase 6 dos jobs: `unit-and-api-tests` (suite completa sobre SQLite) y `postgres-migrations` (levanta `postgres:16`, aplica migraciones y corre los tests marcados `postgres`). En Fase 6.5 el workflow quedó en Estado B: creado y subido al remoto, pero sin ejecución observada desde esta máquina por falta de GitHub CLI y falta de acceso API autenticado. No añadir deploy, build de imagen Docker ni secrets a este workflow sin que el usuario lo pida explícitamente; las credenciales del servicio Postgres en el YAML son valores desechables de una base efímera de CI, no secretos.
